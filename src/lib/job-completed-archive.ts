...
export const COMPLETED_JOBS_PATH = path.join(DATA_DIR, "completed-jobs.xlsx");
...
const SHEETS = {
  ...
  assignees: "CompletedAssignees",
  handovers: "CompletedHandovers",
  partLoans: "CompletedPartLoans",
} as const;
...
export type CompletedJobBundle = {
  job: Job;
  steps: JobStep[];
  events: JobEvent[];
...
async function loadArchiveWorkbook(): Promise<ExcelJS.Workbook> {
  ...
  if (fs.existsSync(COMPLETED_JOBS_PATH)) {
    await wb.xlsx.readFile(COMPLETED_JOBS_PATH);
  }
  return wb;
}
...
async function saveArchiveWorkbook(wb: ExcelJS.Workbook) {
  ensureDataDir();
  const tmp = `${COMPLETED_JOBS_PATH}.${process.pid}.tmp`;
  await wb.xlsx.writeFile(tmp);
  try {
    if (fs.existsSync(COMPLETED_JOBS_PATH)) fs.unlinkSync(COMPLETED_JOBS_PATH);
    fs.renameSync(tmp, COMPLETED_JOBS_PATH);
  } catch {
    fs.copyFileSync(tmp, COMPLETED_JOBS_PATH);
...
function mapJobRow(r: Row): Job {
  return {
    ...
    status: String(r.status || "done") as Job["status"],
...
function mapEventRow(r: Row): JobEvent {
  return {
    id: String(r.id || ""),
    job_id: String(r.job_id || ""),
    type: String(r.type || "completed") as JobEvent["type"],
    note: String(r.note || ""),
    created_at: String(r.created_at || ""),
...
/** Append completed job snapshot to data/completed-jobs.xlsx. */
export async function archiveCompletedJob(input: {
  job: Job;
  steps: JobStep[];
  events: JobEvent[];
  assignees: JobAssignee[];
...
function readAllBundles(wb: ExcelJS.Workbook): CompletedJobBundle[] {
    ...
    .map((r) => {
      ...
      return {
        ...
        part_loans: partLoanRows
          ...
          .map(mapPartLoanRow),
      } satisfies CompletedJobBundle;
    })
    .filter((b): b is CompletedJobBundle => Boolean(b))
    .sort((a, b) =>
...
export async function listCompletedJobDetails(
  liveTechs: Technician[] = []
): Promise<JobWithDetails[]> {
  if (!fs.existsSync(COMPLETED_JOBS_PATH)) return [];
  const wb = await loadArchiveWorkbook();
...
/** Load one completed job and remove it from the archive file. */
export async function takeCompletedJobFromArchive(
  jobId: string
): Promise<CompletedJobBundle | null> {
  if (!fs.existsSync(COMPLETED_JOBS_PATH)) return null;
  const wb = await loadArchiveWorkbook();
  const bundles = readAllBundles(wb);
